...
def demo_stats():
    stayids_train = pd.read_csv('PIPELINE2/data_processed/train_selected.csv')[['stay_id', 'label']]
    stayids_val = pd.read_csv('PIPELINE2/data_processed/val_selected.csv')[['stay_id', 'label']]
    stayids_test = pd.read_csv('PIPELINE2/data_processed/test_selected.csv')[['stay_id', 'label']]
    stayids = pd.concat([stayids_train, stayids_val, stayids_test], axis=0).reset_index(drop=True)
    stayids.columns = ['stay_id', 'label']
    demo = pd.read_csv('PIPELINE2/data_processed/sepsis3_processed.csv')
    ...
    demo.to_csv('PIPELINE2/data_processed/demographic_labels.csv', index=False)
