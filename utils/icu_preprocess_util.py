...
def preproc_proc(dataset_path: str, cohort_path:str, time_col:str, dtypes: dict, usecols: list) -> pd.DataFrame:
    """Function for getting hosp observations pertaining to a pickled cohort. Function is structured to save memory when reading and transforming data_processed."""

    def merge_module_cohort() -> pd.DataFrame:
        """Gets the initial module data_processed with patients anchor year data_processed and only the year of the charttime"""
...
def preproc_chart(dataset_path: str, cohort_path:str, time_col:str, dtypes: dict, usecols: list) -> pd.DataFrame:
    ...
    for chunk in tqdm(pd.read_csv(dataset_path, compression='gzip', usecols=usecols, dtype=dtypes, parse_dates=[time_col],chunksize=chunksize)):
        ...
        chunk_merged.loc[chunk_merged['itemid'].isin([220739, 223900, 223901]), 'valueuom']='points'
        chunk_merged=chunk_merged.dropna() #keeps GCS
        chunk_merged=chunk_merged.drop_duplicates()
        if df_cohort.empty:
            df_cohort=chunk_merged
        else:
            df_cohort=pd.concat([df_cohort, chunk_merged], axis=0, ignore_index=True) #df_cohort.append(chunk_merged, ignore_index=True)
...
def pivot_cohort(df: pd.DataFrame, prefix: str, target_col:str, values='values', use_mlb=False, ohe=True, max_features=None):
    """Pivots long_format data_processed into a multiindex array:
                                            || feature 1 || ... || feature n ||
