...
def ml_eICU2(): 
    ...
    thres_90_list = []
    thres_yuden_list = []
    for idx in range(0, 40): # change to 40 for final run
        print(idx+1, '/', 40)
        df_train_ml = pd.read_csv('/data_processed_eicu/train_' + str(idx+1) + '.csv') # remove index?
        feats_ml = joblib.load('/models/GBM_feat_selection_feat50(40).pkl').feature_names_in_.tolist() + ['label']
        df_train_ml = df_train_ml[feats_ml]
        param_grid = {'learning_rate': 0.1, # median
                'n_estimators': 250, # mode
...
def dl_eICU2():
    ...
    print(1, '/', 40)
    df_train_dl = pd.read_csv('/data_processed_eicu/train_' + str(1) + '_norm.csv') #remove index?
    feats_df = pd.read_csv('/data_processed/train_X_1_norm.csv').iloc[:, :-1].columns.tolist()
    df_train_dl = df_train_dl[feats_df]
    X_df_train_dl = df_train_dl.iloc[:, :-1]
    y = df_train_dl.iloc[:, -1]
    ...
    df_val_dl = pd.read_csv('/data_processed_eicu/val_' + str(1) + '_norm.csv') #remove index?
    ...
    for idx in range(1, 40): # change 
        print(idx+1, '/', 40)

        df_train_dl = pd.read_csv('/data_processed_eicu/train_' + str(idx+1) + '_norm.csv') #remove index?
        feats_df = pd.read_csv('/data_processed/train_X_1_norm.csv').iloc[:, :-1].columns.tolist()
        df_train_dl = df_train_dl[feats_df]
        X_df_train_dl = df_train_dl.iloc[:, :-1]
        y = df_train_dl.iloc[:, -1]
        ...
        df_val_dl = pd.read_csv('/data_processed_eicu/val_' + str(idx+1) + '_norm.csv') #remove index?
        df_val_dl = df_val_dl[feats_df]
        X_df_val_dl = df_val_dl.iloc[:, :-1]
        y = df_val_dl.iloc[:, -1]
        X_t_val = X_df_val_dl.iloc[:, :-10]
    ...
    for idx in range(40): #change
        print(f'\n>>> Dataset chunk {idx + 1} / {40}')
        df_test_dl = pd.read_csv('/data_processed_eicu/test_' +str(idx+1)+ '_norm.csv') #remove index?
        df_test_dl = df_test_dl[feats_df]
        X_df_test_dl = df_test_dl.iloc[:, :-1]
        y = df_test_dl.iloc[:, -1]
        X_t = X_df_test_dl.iloc[:, :-10]
...
def ensemble_eICU2():
    ...
    results = pd.concat([
        pd.DataFrame({'model': 'ML_eICU_balanced_test', 
                      'test_auc': sum(auc_ml_list)/len(auc_ml_list), 'sen_90': sum(sen_90_ml_list)/len(sen_90_ml_list), 'spec_90': sum(spec_90_ml_list)/len(spec_90_ml_list), 'precision_90': sum(precision_90_ml_list)/len(precision_90_ml_list), 'npv_90': sum(npv_90_ml_list)/len(npv_90_ml_list),
                      'sen_yuden': sum(sen_yuden_ml_list)/len(sen_yuden_ml_list), 'spec_yuden': sum(spec_yuden_ml_list)/len(spec_yuden_ml_list), 'precision_yuden': sum(precision_yuden_ml_list)/len(precision_yuden_ml_list), 'npv_yuden': sum(npv_yuden_ml_list)/len(npv_yuden_ml_list)}, index=[0]),
        pd.DataFrame({'model': 'DL_eICU_balanced_test', 
                      'test_auc': sum(auc_dl_list)/len(auc_dl_list), 'sen_90': sum(sen_90_dl_list)/len(sen_90_dl_list), 'spec_90': sum(spec_90_dl_list)/len(spec_90_dl_list), 'precision_90': sum(precision_90_dl_list)/len(precision_90_dl_list), 'npv_90': sum(npv_90_dl_list)/len(npv_90_dl_list),
                      'sen_yuden': sum(sen_yuden_dl_list)/len(sen_yuden_dl_list), 'spec_yuden': sum(spec_yuden_dl_list)/len(spec_yuden_dl_list), 'precision_yuden': sum(precision_yuden_dl_list)/len(precision_yuden_dl_list), 'npv_yuden': sum(npv_yuden_dl_list)/len(npv_yuden_dl_list)}, index=[0])
    ], ignore_index=True)

    # Ensemble - soft voting - average of predictions
    prob = (prob_ml_df + prob_dl_df) / 2
    print(prob)
    ...
    results = pd.concat([results, pd.DataFrame({'model': 'ENSEMBLE_eICU_balanced_test', 
                            'test_auc': sum(auc_list)/len(auc_list), 'sen_90': sum(sen_90_list)/len(sen_90_list), 'spec_90': sum(spec_90_list)/len(spec_90_list), 'precision_90': sum(precision_90_list)/len(precision_90_list), 'npv_90': sum(npv_90_list)/len(npv_90_list),
                            'sen_yuden': sum(sen_yuden_list)/len(sen_yuden_list), 'spec_yuden': sum(spec_yuden_list)/len(spec_yuden_list), 'precision_yuden': sum(precision_yuden_list)/len(precision_yuden_list), 'npv_yuden': sum(npv_yuden_list)/len(npv_yuden_list)}, index=[0])], ignore_index=True)
    results.to_csv('/results/ENSEMBLE_results_eICU_balanced_test.csv', index=False)
