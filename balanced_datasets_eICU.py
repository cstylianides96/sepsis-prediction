...
def df_train_sets_balanced(encoded=False):
    ...
    for idx, split in enumerate(splits):
        ...
        df_train.to_csv('/data_processed_eicu/train_' + str(idx+1) + '.csv', index=False)

def df_train_sets_balanced_norm(encoded=False):
    df_train = pd.read_csv('/data_processed_eicu/train_df_norm.csv')
    df_train['index'] = range(0, len(df_train))
    print(df_train['label'].value_counts())
    ...
    for idx, split in enumerate(splits):
        split = split.reset_index(drop=True)
        df_train = pd.concat([split, cases_train], axis=0).reset_index(drop=True)
        df_train.to_csv('/data_processed_eicu/train_' + str(idx+1) + '_norm.csv', index=False)

def df_val_sets_balanced(encoded=False):
    df_val = pd.read_csv('/data_processed_eicu/val_df.csv')
    ...
    for idx, split in enumerate(splits):
        ...
        df_val.to_csv('/data_processed_eicu/val_' + str(idx+1) + '.csv', index=False)

def df_val_sets_balanced_norm(encoded=False):
    df_val = pd.read_csv('/data_processed_eicu/val_df_norm.csv')
    df_val['index'] = range(0, len(df_val))
    print(df_val['label'].value_counts())
    ...
    for idx, split in enumerate(splits):
        split = split.reset_index(drop=True)
        df_val = pd.concat([split, cases_val], axis=0).reset_index(drop=True)
        df_val.to_csv('/data_processed_eicu/val_' + str(idx+1) + '_norm.csv', index=False)

def df_test_sets_balanced(encoded=False):
    df_test = pd.read_csv('/data_processed_eicu/test_df.csv')
    ...
    for idx, split in enumerate(splits):
        ...
        # print(df_test)
        df_test.to_csv('/data_processed_eicu/test_' +str(idx+1)+'.csv', index=False)

def df_test_sets_balanced_norm(encoded=False):
    df_test = pd.read_csv('/data_processed_eicu/test_df_norm.csv')
    df_test['index'] = range(0, len(df_test))
    print(df_test['label'].value_counts())
    ...
    for idx, split in enumerate(splits):
        ...
        df_test = pd.concat([split, cases_test], axis=0).reset_index(drop=True)
        # print(df_test)
        df_test.to_csv('/data_processed_eicu/test_' +str(idx+1)+ '_norm.csv', index=False)

def create_balanced_datasets_eICU(encoded=False):
    df_train_sets_balanced(encoded=encoded)
    df_val_sets_balanced(encoded=encoded)
    df_test_sets_balanced(encoded=encoded)

    df_train_sets_balanced_norm(encoded=encoded)
    df_val_sets_balanced_norm(encoded=encoded) 
    df_test_sets_balanced_norm(encoded=encoded)     
