...
def run_ensemble():
    ...
    for idx in range(0, 40):
        prob_ml = pd.read_csv('/predictions/ML_prob_balanced_' + str(idx + 1) + '.csv')
        prob_ml_df = pd.concat([prob_ml_df, prob_ml], axis=1)
    prob_ml_df.columns = list(range(1, 41))
    print(prob_ml_df)

    prob_dl_df = pd.read_csv('/predictions/LSTM_15_balanced_prob.csv')
    print(prob_dl_df)
    ...
    prob_avg_df.to_csv('/predictions/GBM-LSTM_balanced_prob.csv', index=False)
    print(prob_avg_df)
    ...
    results.to_csv('/results/ENSEMBLE_results_balanced.csv', index=False)
